...
func TestHTTPConnbuild(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "no upgrade not present when no upgrade configured",
			option: httpConnManagerOptions{
				HTTPUpgradeType: "",
			},
			assertFunc: func(a *assert.Assertions, connManager *xds_hcm.HttpConnectionManager) {
				a.True(connManager.UpgradeConfigs == nil || len(connManager.UpgradeConfigs) == 0)
				a.True(notContainsUpgrade(connManager.UpgradeConfigs, "websocket"))
			},
		},
		{
			name: "websocket upgrade present when configured",
			option: httpConnManagerOptions{
				HTTPUpgradeType: "websocket",
			},
			assertFunc: func(a *assert.Assertions, connManager *xds_hcm.HttpConnectionManager) {
